...
def Connection(i):
    ...
    if cluster_flag:
        ...
        seg_for_synapse = section_cluster(loc)
...
for i in range(len(seg_for_synapse_list)):
    for j in range(len(seg_for_synapse_list)):
        if i < j:
            distance_list.append(h.distance(seg_for_synapse_list[i],seg_for_synapse_list[j]))
            

import seaborn as sns
plt.figure()
sns.distplot(distance_list,hist=False)
